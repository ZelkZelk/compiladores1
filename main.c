...
void help(char *);
void lexer(FILE *,FILE *);			/* Debe recibir FILE para I/O */
...
int main(int argc,char *argv[]){		
	if(argc == 2){				/* Se especifico unicamente input.txt */
		FILE* input = check_fopen(argv[1],"r");

		if(input != NULL){
			lexer(input,stdout);	/* Usamos el puntero stdout como output */
			fclose(input);	
		}		
	}
	else if(argc == 3){ 			/* Se especifico input.txt y output.txt */
		FILE* input = check_fopen(argv[1],"r");
		FILE* output = check_fopen(argv[2],"w+");
...
void lexer(FILE* input, FILE* output){
	...
	/* Imprimimos los tokens, de acuerdo a lo requerido por el ejercicio.
	 * Si el token es un DELIMITADOR_ENTER o __ERROR concatenamos un "\n",
	 * caso contrario concatenamos un espacio en blanco " ".
	 */
	...
	if(head != NULL){
		...
		for(iterator = head;iterator != NULL;iterator = iterator->next_token){
			fprintf(output,"%s",tokentype(iterator->tokentype));

			if(iterator->tokentype == __ERROR || iterator->tokentype == TERMINADOR_ENTER){
				fprintf(output,"\n");
			}
			else if(iterator->tokentype == _EOF){	// Segun el ANSI C, todo archivo debe terminar con un end of line
				fprintf(output,"\n");
			}
			else{
				fprintf(output," ");
			...
			// Descomentar para imprimir el codigo sin espacios en blanco
			// vfprintf(output,"%s", iterator->lexeme);
		}
...
void help(char *bin){
	...
	printf("\tAlternativamente, puede especificar el archivo de salida\n");
